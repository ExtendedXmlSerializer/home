	...
	sealed class InvokeMethod
	{
		...
		public object Invoke(object[] args)
		{
				...
				for (int i = 0; i < methods.Length; i++)
					if (methods[i]
					    ...
					    methods[i]
						    .GetParameters()
						    .Length == args.Length &&
					    CheckParamsCompatibility(methods[i]
						                             .GetParameters(), argTypes, args))
					{
			...
			if (targetMethodInfo == null)
			{
				...
				throw new MissingMemberException(
				                                 (TargetObject is Type ? (Type)TargetObject : TargetObject.GetType())
				                                 .FullName + "." + MethodName);
			}
		...
		protected bool CheckParamsCompatibility(ParameterInfo[] paramsInfo, Type[] types, object[] values)
		{
			for (int i = 0; i < paramsInfo.Length; i++)
			{
				...
				if (values[i] == null && !paramType.GetTypeInfo()
				                                   .IsValueType)
					continue;
				// possible autocast between generic/non-generic common types
				try
				{
					Convert.ChangeType(values[i], paramType, CultureInfo.InvariantCulture);
					continue;
				}
				catch {}
		...
		protected object[] PrepareActualValues(ParameterInfo[] paramsInfo, object[] values)
		{
			...
			for (int i = 0; i < paramsInfo.Length; i++)
			{
				if (values[i] == null || paramsInfo[i]
				                         .ParameterType.IsInstanceOfType(values[i]))
				{
					res[i] = values[i];
					continue;
				}
				...
				try
				{
					res[i] = Convert.ChangeType(values[i], paramsInfo[i]
				...
				catch (Exception error)
				{
					throw new InvalidCastException(
					                               String
						                               .Format("Invoke method '{0}': cannot convert argument #{1} from {2} to {3}",
						                                       MethodName, i, values[i]
							                                       .GetType(), paramsInfo[i]
							                                       .ParameterType), error);
				}
			}
