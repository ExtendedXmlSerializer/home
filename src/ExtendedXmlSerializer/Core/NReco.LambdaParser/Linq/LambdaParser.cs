...
namespace ExtendedXmlSerializer.Core.NReco.LambdaParser.Linq
{
	sealed class LambdaParser
	{
			...
			while (lexem.End < s.Length)
			{
				if (Array.IndexOf(delimiters, s[lexem.End]) >= 0)
				{
					...
					if (lexem.Type != LexemType.StringConstant &&
					    (lexem.Type != LexemType.NumberConstant || s[lexem.End] != '.'))
						return lexem; // stop
				}
				...
				else if (Char.IsControl(s[lexem.End]) && lexem.Type != LexemType.Unknown &&
				         lexem.Type != LexemType.StringConstant)
					return lexem;
					...
					if (lexem.GetValue() == ".")
					{
						...
						if (memberLexem.Type == LexemType.Name)
						{
							...
							else
							{
								...
								{
									End = memberLexem.End,
									Expr = Expression.Call(GetPropertyOrFieldMethod(), val.Expr,
									                       Expression.Constant(memberLexem.GetValue()))
								};
								continue;
							}
		...
		protected ParseResult ParseValue(string expr, int start)
		{
			...
			else if (lexem.Type == LexemType.Name)
			{
				...
				switch (val)
				{
					case "true":
						return new ParseResult()
							{End = lexem.End, Expr = Expression.Constant(new LambdaParameterWrapper(true))};
					case "false":
						return new ParseResult()
							{End = lexem.End, Expr = Expression.Constant(new LambdaParameterWrapper(false))};
					case "new":
						return ReadNewInstance(expr, lexem.End);
				}

				return new ParseResult()
					{End = lexem.End, Expr = Expression.Parameter(typeof(LambdaParameterWrapper), val)};
			}
		...
		protected ParseResult ReadNewInstance(string expr, int start)
		{
			...
			if (nextLexem.Type == LexemType.Name && nextLexem.GetValue()
			                                                 .ToLower() == "dictionary")
			{
				nextLexem = ReadLexem(expr, nextLexem.End);
				if (!(nextLexem.Type == LexemType.Delimiter && nextLexem.GetValue() == "{"))
				...
				do
				{
					...
					if (entryArgs.Count != 2)
						throw new LambdaParserException(expr, nextLexem.Start,
						                                "Dictionary entry should have exactly 2 arguments");
		...
		protected struct Lexem
		{
			...
			public string GetValue()
			{
				if (rawValue == null)
				{
					rawValue = Expr.Substring(Start, End - Start)
					               .Trim();
