	...
	public class CommentParser : IComment
	{
		...
		public Parser<string> SingleLineComment
		{
			get
			{
				...
				return from first in Parse.String(Single)
				       from rest in Parse.CharExcept(NewLine)
				                         .Many()
				                         .Text()
				       select rest;
			}
		...
		public Parser<string> AnyComment
		{
			get
			{
				...
				throw new
					ParseException("Unable to parse comment; check values of fields 'MultiOpen' and 'MultiClose'.");
			}
		}
	}
}