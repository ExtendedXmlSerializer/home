...
{
	sealed class ImmutableSortedDictionariesExtension : ISerializerExtension
	{
		public static ImmutableSortedDictionariesExtension Default { get; }
			= new ImmutableSortedDictionariesExtension();

		ImmutableSortedDictionariesExtension()
			: this(new IsAssignableGenericSpecification(typeof(ImmutableSortedDictionary<,>))) {}

		readonly ISpecification<TypeInfo> _specification;

		public ImmutableSortedDictionariesExtension(ISpecification<TypeInfo> specification)
		...
		{
			public object Get() => this.ToImmutableSortedDictionary();
		}

		sealed class GenericTypes : IGenericTypes
		{
			readonly static TypeInfo Check = typeof(ImmutableSortedDictionary).GetTypeInfo();
			readonly static ImmutableArray<TypeInfo> Type = typeof(ImmutableSortedDictionary<,>).GetTypeInfo()
			                                                                                    .Yield()
			                                                                                    .ToImmutableArray();
		...
		sealed class Contents : IContents
		{
			...
			sealed class Reader<TKey, TValue> : IReader
			{
				...
				public object Get(IFormatReader parameter)
					=> _reader.Get(parameter).AsValid<IDictionary<TKey, TValue>>().ToImmutableSortedDictionary();
			}
		}
	}
}